...
export function displayResults(data) {
    ...
    if (header.message === 'Bad Request' && isValid) {
        table = new Table({
            head: ['Unfortunately'],
            colWidths: [40],
        });
        ...
        info.forEach(item => {
            ...
            keys.forEach((key, index) => {
                ...
                if (key === 'valid') {
                    ...
                    const localFormat = values[index].local;
                    tableRow.push(localFormat);
                    // Phone country
                } else if(key === 'country') {
                    const country = values[index].name;
                    tableRow.push(country);
